...
## 使用
- **页面自助添加**：点右上角「+ 添加」→ 填名称 + 网址（或勾选 NAS 设备填内网+外网）→ 选「分类」。分类为**真正的下拉框**：列出当前+另一模式已有的分类与「我的收藏」，也可选「＋ 新建分类…」手输新名。**输入网址后会按域名自动预选分类**（如填 xiaohongshu.com 自动选中「资讯」），可手动改。链接存浏览器本地，图标自动抓；链接上「×」可删。不选则归「我的收藏」。
- **右键分类快速添加**：在任意分类标题上**右键** → 直接弹出「添加网址」并**自动选中该分类**（普通分类填单网址；NAS 分类右键则自动勾选「NAS 设备」填内网+外网）。比从顶部「+ 添加」再选分类更顺手。
- **高级 / 批量**：直接编辑 `sites.js`：
